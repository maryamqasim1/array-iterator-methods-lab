...
veryOldInventors = inventors.filter(x => {
    if (x.year >= '1500' && x.year <= '1599')
        return x;
});
...
inventorNamedAda = inventors.find(x => {
    if (x.first == 'Ada')
        return x;
})
...
// Enter your solution code here:
isAdultPresent = devs.some(x => {
    age = new Date().getFullYear() - x.year // current year
    return age >= 18;
})
...
// Enter your solution code here:
isEveryone19OrOlder = devs.every(x => {
    age = new Date().getFullYear() - x.year // current year
    return age >= 19;
})
...
// Enter your solution code here:
totalYearsLived = inventors.reduce((total, x) => {
